...
module.exports = {
	...
	// aliases: [''],
	// Descriptions
	image: 'https://i.imgur.com/Zoc1aLd.png',
	description: english.cmdDescriptions.imgs.dog,
	descriptionS: spanish.cmdDescriptions.imgs.dog,
	descriptionG: german.cmdDescriptions.imgs.dog,
	descriptionF: french.cmdDescriptions.imgs.dog,
	descriptionN: nor.cmdDescriptions.imgs.dog,
	// Usage
	usage: 'dog || dog fact',
	// Status
...
async function English1(client, message, args, lang, prefix, modeR, adminR, logsC) {
	async function getDog() {
		const r = await fetch('https://some-random-api.ml/img/dog');
		r.json().then((e) => {
	...
	async function getfact() {
		const e = await fetch('https://some-random-api.ml/facts/dog');
		e.json().then((e) => {
			message.channel.send(`*${e.fact}*`);
		});
	}
	if (!args.length) getDog();
